...
open class ThemeManager: NSObject {
    ...
    open private(set) var currentTheme      : NSDictionary?
    open private(set) var currentThemePath  : ThemePath?
    open private(set) var currentThemeIndex : Int = 0
    
    open func setTheme(index: Int) {
        currentThemeIndex = index
        NotificationCenter.default.post(name: Notification.Name(rawValue: ThemeUpdateNotification), object: nil)
    }
    
    open func setTheme(plistName: String, path: ThemePath) {
